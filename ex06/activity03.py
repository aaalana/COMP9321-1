...
# The @api.response() decorator allows you to document the known responses and /
# is a shortcut for @api.doc(responses='...').



app = Flask(__name__)
api = Api(app, default="Books",  # Default namespace
...
class BookList(Resource):
    ...
    def get(self):
        ...
        # return True if specific key value not exit
        ascending = args.get('ascending', True)

        if order_by:
            df.sort_values(by=order_by, inplace=True, ascending=ascending)

        #orient: indicate the expected JSON string format, defalt for df is 'column'
    ...
    def post(self):
        ...
        id = book['Identifier']
        if id in df.index:
            return {"message": "Book with {} id is already exist in database".format(id)}, 400
